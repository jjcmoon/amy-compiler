...
class Function private (val name: String, val args: Int, val isMain: Boolean, val locals: Int, val index: Int, val code: Code) {
...
object Function {
  def apply(name: String, args: Int, isMain: Boolean, index: Int)(codeGen: LocalsHandler => Code) = {
    val lh = new LocalsHandler(args)
    // Make code first, as it may increment the locals in lh
    val code = codeGen(lh)
    new Function(name, args, isMain, lh.locals, index, code)
  }
}
